...
class DataController: ObservableObject {
    ...
    func getAllData() -> [ChoreDataModel] {
        let fetchRequest1: NSFetchRequest<ChoreDataModel> = ChoreDataModel.fetchRequest()
        
        do {
            return try container.viewContext.fetch(fetchRequest1)
        } catch {
            return []
        }
    }
